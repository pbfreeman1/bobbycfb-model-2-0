...
export function scorePSS({ tierName, m, qualifies }, game) {
  ...
  // The model's own fair line, in the same raw (home-positive) convention as
  // consensus_spread / vegas_line. Convert with spreadForSide-style logic at
  // display time to get the correctly-signed number for whichever side is
  // suggested — never display this raw value directly.
  const suggestedLine = m.consensus;
