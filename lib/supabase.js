...
// Resolves the "current" week for a season as the highest week number that
// has any games in the games table. Used to default week selectors instead
// of hardcoding week=1, while still leaving the selector free to go back to
// earlier weeks. Falls back to 1 if the lookup fails or nothing is found.
export async function getCurrentWeek(season) {
  try {
    const rows = await sbFetch(`games?select=week&season=eq.${season}&order=week.desc&limit=1`);
    return rows.length ? rows[0].week : 1;
  } catch {
    return 1;
  }
}
