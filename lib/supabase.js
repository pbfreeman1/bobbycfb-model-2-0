...
// has FINAL games. This prevents the default from jumping ahead to a future
// scheduled week before those games have been played and graded.
// Falls back to 1 if the lookup fails or nothing is found.
export async function getCurrentWeek(season) {
  try {
    const rows = await sbFetch(`games?select=week&season=eq.${season}&status=eq.final&order=week.desc&limit=1`);
